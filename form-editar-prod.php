    ...
    <body class="app">
        ...
        <div class="flex">
            ...
            <div class="content">
                ...
                <div class="top-bar">
                    <!-- BEGIN: Breadcrumb -->
                    <div class="-intro-x breadcrumb mr-auto hidden sm:flex"> <a href="" class="">Menu</a> <i data-feather="chevron-right" class="breadcrumb__icon"></i> <a href="" class="breadcrumb--active">Editar Produto</a> </div>
                    <!-- END: Breadcrumb -->
                    <!-- BEGIN: Account Menu -->
                    <?php
                ...
                <div class="grid grid-cols-12 gap-6">
                    <div class="col-span-12 xxl:col-span-9 grid grid-cols-12 gap-6">
                        ...
                        <div class="col-span-12 mt-8">
                            ...
                            if(isset($_GET['id'])){
                                $prodid = $_GET['id'];
                                $query = "SELECT * from produto WHERE codprod = '$prodid'";
                                $result = mysqli_query($link, $query);
                                $row = mysqli_fetch_array($result);

                                $fcodprod = $row["codprod"];
                                $ffornecedor = $row["fornecedor"];
                                $fcategoria = $row["categoria"];
                            
                            ?>
                            <form action="form-editar-prod.php" method="post">
                                <div class="intro-y box">
                                    ...
                                    <div class="p-5" id="input">
                                        <div class="preview">
                                            <div>
                                                <input name = "prodid" value="<?php echo $prodid?>" hidden>
                                            </div>
                                            <div>
                                                <label>Designação</label>
                                                <input name="designacao" value="<?php echo $row["designacao"] ?>" type="text" class="input w-full border mt-1 mb-4" placeholder="Designação do produto">
                                            </div>
                                            <div>
                                                <label>Quantidade</label>
                                                <input name="quantidade" value="<?php echo $row["quantidade"]?>" type="text" class="input w-full border mt-1 mb-4" placeholder="Quantidade do produto">
                                            </div>
                                            <div>
                                                <label>Cor</label>
                                                <input name="cor" value="<?php echo $row["cor"]?>" type="text" class="input w-full border mt-1 mb-2" placeholder="Cor do produto">
                                            </div>
                                            <div>
                                                <label>Preço unitário</label>
                                                <input name="precounitario" value="<?php echo $row["precounitario"]?>" type="text" class="input w-full border mt-1 mb-4" placeholder="Preço do produto">
                                            </div>
                                            <div>
                                                <label>Fornecedor</label>
                                                <select name="idforn" class="input border mr-2">
                                                <?php
                                                        $qry = "SELECT * from fornecedores where estado = 1 and idforn = '$ffornecedor'";
                                                        ...
                                                        ?>
                                                            ...
                                                            <?php } ?>
                                                    <?php
                                                        $qry = "SELECT * from fornecedores where estado = 1 and idforn != '$ffornecedor' order by idforn";
                                                        $result = mysqli_query($link, $qry);
                                                        while ($row = mysqli_fetch_array($result)) {
                                                        ?>
                                                            <option value="<?php echo $row['idforn'] ?>">
                                            ...
                                            <div>
                                                ...
                                                <?php
                                                        $qry = "SELECT * from categorias where estado = 1 and id = '$fcategoria'";
                                                        $result = mysqli_query($link, $qry);
                                                        while ($row = mysqli_fetch_array($result)) {
                                                        ?>
                                                            ...
                                                            <?php } ?>
                                                    <?php
                                                        $qry = "SELECT * from categorias where estado = 1 and id != '$fcategoria' order by id";
                                                        $result = mysqli_query($link, $qry);
                                                        while ($row = mysqli_fetch_array($result)) {
                                                        ?>
                                                            <option value="<?php echo $row['id'] ?>">
                            ...
                            <?php } ?>                                                                        
                            <?php
                                if(isset($_POST["prodid"]) && isset($_POST["idforn"]) && isset($_POST["precounitario"]) && isset($_POST["designacao"]) && isset($_POST["quantidade"]) && isset($_POST["cor"])){
                                    $fprodid = $_POST["prodid"];
                                    ...
                                    $query = mysqli_query($link, "UPDATE produto, fornecedores, categorias SET designacao = '$fdesignacao', quantidade = '$fquantidade', cor = '$fcor', precounitario = '$fprecounitario', fornecedor = '$fidforn', categoria = '$fid' WHERE codprod = '$fprodid'");
                                    if($query){
                                        echo "<div class=\"rounded-md px-5 py-4 mb-2 bg-theme-9 text-white\">Editado com sucesso</div>";

                                        $query = mysqli_query($link, "INSERT INTO logs (user, descricao, datahora, userid) VALUES ('$user_name', 'Editou Produto', now(), '$iduser')");
                                        ?>
                                            <script>
